cd%% A small exercize on STA and LNP model fitting, by U. Ferrari

% Here the goal is to compute the STA from the response to a set of
% checkerboad images. 

% Note that the code is not intended to be optimized
% for computational performance. In particular, most of the for cycles can
% be rewrittenm as vector multiplications which in Matlab are usually much
% faster. [[Brave students can give it a try, and optimize the code]]
...
for n=1:N
   signal(n) = sum(sum( sta .* stimulus(:,:,n) ) ); 
end


% plot signal versus response
figure('Name', 'filter versus spikes')
plot(signal,response, 'k.');    % Scatter plot of signal (filtered stimulus) versus spike count  
hold on
...
nBin = 50;
[COUNT,EDGES,BIN] = histcounts(signal,nBin);
...
for bin = 1:nBin
   anchor_y(bin) = mean(response(BIN==bin)); 
end
...
% ppval allows for evaluating the spline interpolation at any given point points
predictions = ppval(nonlinearity,signal);

% scatterplot between the responses for all images and the predictions 
figure('Name', 'spike count versus prediction')
hold on
plot(response, predictions, 'k.','MarkerSize',14);              % ex 2(d) compare predicted and observed spike count
plot([0, 120], [0, 120], 'k--'); 
ylim([0 max(response)*1.05]);
